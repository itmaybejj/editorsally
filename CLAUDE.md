...
- **JavaScript**: `assets/theme.js` handles nav population, language routing, active link highlighting, language picker, and responsive offcanvas menu. `lang/i18n.js` provides i18n configuration (path mappings, nav strings, native language names, supported languages) consumed by theme.js. `assets/demo.js` powers the live accessibility checker demos.
...
All content pages follow the structure in `template.html`. Pages live at `en/<topic>/index.html` (except "about" which is at `en/index.html`). Each page's `<header>` contains a `<div id="lang-picker"></div>` placeholder that `theme.js` populates with a language-switching dropdown. Navigation links are dynamically injected by `theme.js` using strings from `lang/i18n.js`.
